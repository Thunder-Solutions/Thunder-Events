...
const init = async (init = DEFAULT_FETCH_DATA) => {
  ...
  if ('toggleHiddenLocation' in data) {
    if (!checkType(data.toggleHiddenLocation, 'function')) throw 'The `toggleHiddenLocation` field expects a function'
    state.toggleHiddenLocation = data.toggleHiddenLocation
  }
...
const toggleHiddenLocation = async location => {
  const hiddenLocations = await state.toggleHiddenLocation(location)
