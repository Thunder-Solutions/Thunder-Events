...
const getFavorites = async () => {
  ...
  const favorites = await state.getFavorites(events)
  const matchEvent = cEvent => {
    const match = events.find(event => event.id === cEvent.id)
    if (typeof match !== 'undefined') return match

    // adjust old data to match the new format
    const matchByName = events.find(event => event.name === cEvent.name)
    if (typeof matchByName !== 'undefined') {
      state.removeFavorite(matchByName).then(() => {
        state.addFavorite(matchByName)
      })
      return true
    } else {
      state.removeFavorite(cEvent)
      return false
    }
  }
  return favorites.filter(matchEvent).sort((pf, nf) => {
