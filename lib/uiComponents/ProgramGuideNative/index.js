...
import { getDateInfo, dateComparison } from '../../utilities'
...
class ProgramGuideNative extends HTMLElement {
  ...
  constructor() {
    ...
    component.waitForFetch = getLocations().then(async locations => {
      ...
      const activeDay = dateComparison(currentDate, Object.keys(days)[0]) !== -1 && dateComparison(currentDate, Object.keys(days)[days.length - 1]) !== 1 ?
        '' + currentDate.toLocaleString('en-US', { weekday: 'long' }) + ' ' + new Intl.DateTimeFormat('en-US').format(currentDate) : Object.keys(days)[0]
  ...
  setState(state, options = {}) {
    ...
    if (_options.render) component.render()
