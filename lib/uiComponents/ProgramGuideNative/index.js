...
class ProgramGuideNative extends HTMLElement {
  ...
  constructor() {
    ...
    component.waitForFetch = getLocations().then(async locations => {
      ...
      // update the default state with the async results
      // defined by the dev using the package
      ...
      setImageData(programGuideData, component.templateCache) // keep loaded image in memory
  ...
  async addFavorite(encodedEvent) {
    ...
    component.setState({
      favorites: [...component.#state.favorites, event],
    })
  ...
  async removeFavorite(encodedEvent) {
    ...
    const favorites = [...component.#state.favorites]
  ...
  toggleFloorPlanInteractive() {
    const component = this
    const floorPlanInteractive = !component.#state.floorPlanInteractive
    const { floorPlan } = component.#state
    ...
    component.setState({ floorPlanInteractive })
  }
  ...
  openEventDialog(encodedEvent) {
    ...
    const eventJSON = decodeURIComponent(encodedEvent)
    const event = JSON.parse(eventJSON)
    component.setState({ activeEvent: getActiveEvent(event) }, { render: false })
  ...
  changeSearch({ target }) {
    const component = this
    component.setState({ currentSearch: target.value }, { render: false })
  }
