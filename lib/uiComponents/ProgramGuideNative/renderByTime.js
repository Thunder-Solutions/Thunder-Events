...
const renderByTime = ({ events, times, locations, sortBy }) => {
  ...
  return mapString(Object.keys(days), day => {
    ...
    const now = getDateInfo(roundMinutes(Date.now(), interval))
    const allTimes = sortBy === 'now'
      ? _times.filter(({ date }) => Number(date) >= now.date)
      : sortBy === 'nowOnly'
        ? _times.filter(({ date }) => Number(date) === now.date)
        : _times
    const times = allTimes.length ? allTimes : [{ date: now.date, timeString: now.time }]
    return /* html */`
