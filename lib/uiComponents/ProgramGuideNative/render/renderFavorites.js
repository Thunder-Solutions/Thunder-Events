...
const renderFavorites = ({ favorites, events }) => {
  const eventExists = event => !!events.find(e => {
    return JSON.stringify(e) === JSON.stringify(event)
  })
  favorites.sort((prevEvent, nextEvent) => {
    const prevTime = new Date(prevEvent.start).getTime()
    const nextTime = new Date(nextEvent.start).getTime()
    return prevTime > nextTime ? 1 : prevTime < nextTime ? -1 : 0
  })
  return /* html */`
        ...
        <tbody class="pg-body">
          ${favorites.length === 0
            ? /* html */`
              ...
              `
            : mapString(favorites.filter(eventExists), event => {
