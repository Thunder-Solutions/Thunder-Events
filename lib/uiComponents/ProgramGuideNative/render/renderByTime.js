...
import { getDateInfo, getEventId, mapString, roundMinutes, runMethod, trim, toKebabCase } from '../../../utilities'
        ...
        <thead class="pg-table-header">
          <tr class="pg-row">
            <td class="pg-cell-wrapper pg-cell-wrapper--corner pg-cell-wrapper--corner-collapsed" onclick="${runMethod('toggleDateDialog', true)}">
              <div class="pg-cell pg-cell--corner"><span class="pg-corner-text">${currentDay}</span></div>
            </td>
          ...
          ${mapString(locations, _location => /* html */`
            <tr class="pg-row">
              ...
              ${mapString(times, ({ date }) => {
                ...
                const bgColor = event?.category ? 'background-color: var(--color-event-' + toKebabCase(event.category) + ', var(--color-cell));' : ''
                if (blank) return '' // blank if cell from previous row occupies this space
                return /* html */`
                  <td
                    style="${bgColor.length > 0 ? bgColor : ''}"
