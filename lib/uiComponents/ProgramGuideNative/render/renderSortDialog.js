...
const renderSortDialog = ({ locations, hiddenLocations, times, activeDay, activeTime: _activeTime }) => {
      ...
      <section class="pg-dialog-content">
        ...
        <menu class="pg-dialog-menu">
          ...
          ${mapString(locations, _location => /* html */`
            <button class="pg-dialog-menu-item ${hiddenLocations.includes(_location) ? 'pg-dialog-menu-item-hidden' : ''}" onclick="${runMethod('hideLocation', _location)}">${_location}</button>
          `)}
        </menu>
        <menu class="pg-dialog-menu">
