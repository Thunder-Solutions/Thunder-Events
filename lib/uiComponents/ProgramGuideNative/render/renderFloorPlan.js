...
const renderFloorPlan = (state, setState, component) => {
  const { floorPlan, floorPlanInteractive, marker, locations, hiddenLocations } = state
  const { root, toggleHiddenLocation } = component
  const { title, clickableAreas, dimensions } = floorPlan
  const { height, width } = dimensions
  const showMap = floorPlanInteractive
  ...
  // apply event listeners after rendering
  requestAnimationFrame(() => {
    if (!showMap) return
    for (const { href } of clickableAreas) {
      const area = root.querySelector(`area[href="${href}"]`)
      if (area === null) continue
      area.addEventListener('click', event => {

        // only handle location hashes
        if (!href.startsWith('#View-Location-')) return

        // avoid conflicting with global href listeners
        event.preventDefault()

        // corresponding location to href
        const _location = locations.find(l => {
          const lookup = href.replace('#View-Location-', '')
        ...
        if (hiddenLocations.includes(_location)) {
          toggleHiddenLocation(_location, true)
        }
        ...
        setState({
          ...
          marker: _location,
          view: 'guide',
          scrollTo: '.pg-cell-wrapper--column-header-active-location',
          sortBy: 'location',
        })
      })
    }
