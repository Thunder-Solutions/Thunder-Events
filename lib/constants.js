...
export const DEFAULT_HIDE_LOCATION = async location => {
  const hiddenLocations = JSON.parse(localStorage.getItem(HIDDEN_LOCATIONS_KEY) || '[]')

  if (hiddenLocations.includes(location)) {
    const removeIndex = hiddenLocations.findIndex((loc) => {
      return location === loc
    })
    const newHiddenLocations = [...hiddenLocations]
    newHiddenLocations.splice(removeIndex, 1)
    
    localStorage.setItem(HIDDEN_LOCATIONS_KEY, JSON.stringify(newHiddenLocations))
...
export const DEFAULT_FETCH_DATA = () => Promise.resolve({
  ...
  toggleHiddenLocation: DEFAULT_HIDE_LOCATION,
